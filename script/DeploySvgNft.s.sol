...
contract DeploySvgNft is Script{
    ...
    function run() public returns(SvgNft) {
        string memory sadSvg = vm.readFile("./images/sad.svg");
        string memory happySvg = vm.readFile("./images/happy.svg");
        console.log(sadSvg);
        console.log(happySvg);
        vm.startBroadcast();
        svg = new SvgNft(svgToImageUri(sadSvg), svgToImageUri(happySvg));
        vm.stopBroadcast();
        return svg;
    }

    /**
     * @notice this is doing essentially the same as base64 -i in the terminal and prepending some data
     * @param svg is the svg file that will be turned into base64 format
     */
